...
object MainData extends App {
  ...
  val normalizedS = s.map(rec =>
    NormalizedStationRecord(
      ...
      timestamp = OffsetDateTime.parse(rec.recordTimestamp)
    )
  )

  // Describe dataset
  val nbDates = normalizedS
    .groupBy(rec => (rec.timestamp.getDayOfYear, rec.timestamp.getYear))
    .size
  ...
  val bestStation =
    gbPerStationPerHourAvailableBikesMean
      ...
      ._1
  println(
    s"$bestStation has the best Velib availability " +
  ...
  val gbPerStationPerHourAvailableDocksMean = gbPerStation
    .mapValues(listRec =>
      listRec
        .groupBy(_.timestamp.getHour)
        .mapValues(l => computeMean(l.map(_.nbFreeDock)))
    )

  // Then we need to find the max of these means to get the hour and the
  // station with highest dock availability
  val bestDockHour = gbPerStationPerHourAvailableDocksMean
    .mapValues(_.maxBy(_._2))
    .maxBy(_._2._2)
    ._2
    ._1
  val bestDockAvailability = gbPerStationPerHourAvailableDocksMean
    .mapValues(_.maxBy(_._2))
    .maxBy(_._2._2)
    ._2
    ._2
  val bestDockStation = gbPerStationPerHourAvailableDocksMean
    .mapValues(_.maxBy(_._2))
    .maxBy(_._2._2)
    ._1

  println(
    s"$bestDockStation has the best dock availability " +
  ...
  val gbPerStationDailyMean =
    gbPerStation.mapValues(listRec => computeMean(listRec.map(_.nbBike)))
  val threeBestStations = gbPerStationDailyMean.toSeq
    .sortBy(_._2)
    .reverse
    .take(3)
  threeBestStations.foreach(rec =>
    println(
      s"${rec._1} is in top-3, with daily " +
