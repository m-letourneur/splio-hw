...
object MainVelibAvailability extends App {
  ...
  def getConsolidateRelevantStations(sList: List[StationStatus]) = {
    for {
      ...
      listRelevantStationInformation
        .map { sInfo =>
          {
            val sStatusOption = sList
              .filter(_.stationCode == sInfo.stationCode)
              .headOption
  ...
  def printConsolidatedStation(station: Station) = {
    println(
      s"${station.status.numBikesAvailable} bikes available at station" +
        s" ${station.info.name}"
    )
  }
